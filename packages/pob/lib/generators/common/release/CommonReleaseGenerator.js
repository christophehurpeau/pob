...
export default class CommonReleaseGenerator extends Generator {
  ...
  writing() {
    ...
    if (isReleasePleaseEnabled && this.options.packageNames) {
      ...
      if (!this.fs.exists(this.templatePath('.release-please-manifest.json'))) {
        writeAndFormatJson(
          this.fs,
          this.destinationPath('.release-please-manifest.json'),
          Object.fromEntries(
            packageLocations.map((packagePath) => [
              packagePath,
              this.fs.readJSON(`${packagePath}/package.json`, {}).version,
            ]),
          ),
        );
      }
