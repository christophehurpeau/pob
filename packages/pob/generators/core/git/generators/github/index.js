...
module.exports = class GitHubGenerator extends Generator {
  constructor(args, opts) {
    ...
    if (!GITHUB_TOKEN) {
      console.error('Missing POB_GITHUB_TOKEN. Create one with https://github.com/settings/tokens/new?scopes=repo&description=POB%20Generator and add it in your env variables.');
      process.exit(1);
    }

    if (!CIRCLECI_TOKEN) {
      console.error('Missing POB_CIRCLECI_TOKEN. Create one with https://circleci.com/account/api and add it in your env variables.');
      process.exit(1);
    }
  }
  ...
  async end() {
    ...
    if (this.options.shouldCreate) {
      try {
        ...
        this.spawnCommandSync('git', ['commit', '-m', 'chore: initial commit [skip ci]'], { cwd });
        this.spawnCommandSync('git', ['push', '-u', 'origin', 'master'], { cwd });
        ...
        if (this.fs.exists('.circleci/config.yml')) {
          try {
            await got.post(`https://circleci.com/api/v1.1/project/github/${owner}/${repo}/follow`, { query: { 'circle-token': CIRCLECI_TOKEN }, json: true });


            try {
              const result = await got.get(`https://circleci.com/api/v1.1/project/github/${owner}/${repo}/checkout-key`, { query: { 'circle-token': CIRCLECI_TOKEN }, json: true, body: { type: 'deploy-key' } });
