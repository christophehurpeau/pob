...
module.exports = class extends Generator {
  ...
  initializing() {
    ...
    this.options.entries.split(',').forEach(entry => {
      const entryDestPath = this.destinationPath(
        this.options.destination,
        `${entry}.js`
      );
      if (
        this.options.env_node6 ||
        this.options.env_node7 ||
    ...
    const indexSrcDestPath = this.destinationPath(
      this.options.destination,
      'src/index.js'
    );
    if (!this.fs.exists(indexSrcDestPath)) {
      const idxJsxDestPath = this.destinationPath(
        this.options.destination,
        'src/index.jsx'
      );
    ...
    if (this.options.flow) {
      ...
      const typesDestPath = this.destinationPath(
        this.options.destination,
        'src/types.js'
      );
  ...
  writing() {
    ...
    if (pkg['webpack:browser'])
      pkg['module'] = pkg['webpack:browser'].replace('webpack', 'module');
    if (pkg['webpack:browser-dev'])
      pkg['module-dev'] = pkg['webpack:browser-dev'].replace(
        'webpack',
        'module'
      );
    if (pkg['webpack:main'])
      pkg['module'] = pkg['webpack:main'].replace('webpack', 'module');
    if (pkg['webpack:main-dev'])
      pkg['module-dev'] = pkg['webpack:main-dev'].replace('webpack', 'module');
    if (pkg['webpack:node']) pkg['module:node'] = pkg['webpack:node'];
    if (pkg['webpack:node-dev'])
      pkg['module:node-dev'] = pkg['webpack:node-dev'];

    // fix
    if (pkg['module'])
      pkg['module'] = pkg['module'].replace('webpack', 'module');
    if (pkg['module-dev'])
      pkg['module-dev'] = pkg['module-dev'].replace('webpack', 'module');
    ...
    packageUtils.addDependency(pkg, 'flow-runtime', '^0.10.0');
    ...
    if (
      ...
      packageUtils.addDevDependency(pkg, 'babel-preset-env', '^1.3.2');
    } else {
    ...
    if (
      ...
      packageUtils.addDevDependency(pkg, 'babel-preset-latest-node', '^0.2.2');
    } else {
    ...
    ['', '-modern-browsers', '-node'].forEach(middle => {
      ['', '-dev'].forEach(suffix => {
        ...
        if (pkg[`webpack:${key}`]) {
          pkg[`module:aliases${middle === '' ? '-browser' : middle}${suffix}`] = pkg[`webpack:${key}`].replace('webpack', 'module');
    ...
    if (
      ...
      const aliases = this.options.entries
        .split(',')
